...
async def rag_ingest_pdf(ctx:inngest.Context):
    ...
    ingested= await ctx.step.run("embed-and-upstep",lambda :_upsert(chunks_and_src),output_type=RAGUpsertResult)
    return ingested.model_dump()

@inngest_client.create_function(
    fn_id="RAG: Query PDF",
    trigger=inngest.TriggerEvent(event="rag/inngest_pdf_ai")
)
...
async def rag_query_pdf_ai(ctx:inngest.Context):
    ...
    found=await ctx.step.run("embed-and-upstep",lambda : _search(question,top_k),output_type=RAGSearchResult)
    ...
    res=await ctx.step.ai.infer(
        ...
        body={
            ...
            "temperature":0.2,
            "messages":[
                {"role":"system","context":"you answer question using only the provided context"},
